...
證交所的 `STOCK_DAY_ALL` 只提供**已收盤的完整交易日**資料，當日的要等收盤結算後才發布（約 15:30 之後）。所以：

- 盤中看到的是**上一個交易日**的收盤價
- 週末與假日看到的是**上週五**的收盤價

實際案例：使用者 9/7（週一）下午 3:30 看到報價停在 9/4，以為系統壞了。實測證交所當下回傳的 `Date` 就是 `1150904`——**不是快取問題，手動刷新也拿不到更新的資料**。

因此持股頁明確標示「這是收盤價，不是盤中即時報價」。若真的需要盤中價，得換一個提供即時報價的來源（多半要付費或有嚴格的頻率限制），對個人淨值檢視不值得。

### 資產卡把台股與複委託分開顯示

投資部位在報表頁拆成兩列，**報酬率各自計算**：

```
投資      NT$ 771,912   （台股 + 複委託換算台幣）
  台股（2 檔）      NT$ 723,540   +NT$ 268,454（+59.0%）
  複委託（2 檔）    US$ 1,529.22  +US$ 102.58（+7.2%）
```

三個判斷：

- **報酬率必須分開算。** 台股 +59% 與美股 +7% 混在一起會變成一個看不出誰在拖累的數字。
- **美股用原幣別算報酬率**，不受匯率波動污染——換不同匯率時 `gainRatio` 完全不變，有測試守著。
- **總資產維持全部換算台幣。** 換算過的合計才有意義；再拆幣別只會讓人分不清哪個是原幣、哪個是換算值。

順帶移除了原本的「合計成本 + 未實現損益」那一行：它把兩種幣別相加，資訊價值低，而且跟分市場明細重複。

### 手動刷新報價

持股頁有「更新報價」按鈕，走 `getQuotes(wanted, { force: true })` 略過 15 分鐘的快取。有用的場合：

1. 當日收盤結算後，不必等快取過期就能拿到今天的價格
2. 剛新增持股時立刻取得報價
3. 懷疑數字沒更新時，有個明確的動作可以確認

實測：一般呼叫 194 ms（讀快取），force 呼叫 3,023 ms（打外部 API），`fetchedAt` 確實更新。
...
`RecurringTemplate` 從第一天就在 schema 裡（SPEC 原本的 Phase 1.5），現在用來解決一個實際的計算錯誤。

### 解決的問題

「每日可用額度」原本是 `(預算 − 已花消費) ÷ 剩餘天數`，**沒有把「還沒付但跑不掉」的固定支出扣掉**。

以預算 24,630、房租 12,000 在 5 號扣、當月 30 天為例：

| 日期 | 修正前 | 修正後 |
|---|---|---|
| 3 號（房租未付，已花 900） | 每天 **848** | 每天 **419** |
| 6 號（房租已付，累計 12,900） | 每天 469 | 每天 469 |

修正前月初會**系統性高估一倍**，然後在記房租那天腰斬——不是處境變了，是之前算錯了。這種跳動會讓人不信任這個數字。

### 預估固定支出的來源

```
預估固定支出 = max(啟用中的範本金額合計, 近三個月的固定支出平均)
本月尚未發生 = max(0, 預估固定支出 − 本月已記錄的固定支出)
```

取較大者、且只扣「尚未發生」的部分，兩者都是為了避免高估可用額度——**高估比低估危險**（你會以為還有錢可以花）。

範本是使用者自己填的精確值，第一個月就能用；歷史平均是沒填範本時的後備。

### 消費速度的日均只能算變動消費

`monthPace` 原本收 `consumptionSoFar`（變動 + 固定），日均直接除以已過天數。房租是某一天的一大筆，混進日均再乘天數會荒謬地高——實際案例：9/1 記了房租 7,000 + 午餐 345，月底預測變成 **220,350**。

改成分開傳 `variableSoFar` 與 `fixedSoFar`：

```
日均      = 變動消費 ÷ 已過天數        ← 不含固定支出
月底預測  = 日均 × 當月天數 + 預估整月固定支出
```

同一個案例修正後是 **20,207**（345 × 30 + 9,857）。
...
**Tailwind v4 的設定方式不同**：不是 `tailwind.config` 的 `darkMode: "class"`，而是在 CSS 裡宣告 `@custom-variant dark (&:where(.dark, .dark *));`。

**顏色是機械式映射的**（434 處、33 個檔案），不可能手工改。腳本用一張「淺色 → 深色」對照表批次加上 `dark:` 變體，原則是讓層次關係維持不變：白卡片浮在灰頁面上 → 深灰卡片浮在更深的頁面上。

**一個映射的例外**：`bg-slate-900` 對按鈕該翻成亮色（深底上的主要按鈕要醒目），但對主視覺卡片不該——那 6 張卡片翻成亮白會變成刺眼的白塊，改成 `dark:bg-slate-800` 並保持淺色文字。批次腳本無法從 class 區分兩者，只能事後手動修正。

`themeColor` 也改成隨主題切換，否則深色頁面上方會露出一條淺色的網址列。

### 資料不足時顯示「—」，不要顯示看起來很有把握的錯誤數字

三個實際踩到的案例，共通根因都是「拿不完整的資料當完整的用」：

**① 月平均要排除「開始記帳的第一個月」。** 使用者 8/25 才開始記，8 月只有 7 天共 5,857 元。當成月消費會讓緊急預備金顯示 **25.6 個月**，實際上只有約 6 個月——**高估安全感是最危險的方向**，那個指標存在的唯一目的就是在現金太薄時示警。`usableMonths()` 同時排除當月（沒過完）與最早有資料的月份（幾乎必然不完整），沒有可用月份就回傳 null。

**② 已過天數太少時不做外推。** 9/1 花了 810 元，乘上 30 天變成 24,300；多吃一頓 500 元的預測就跳到 39,000。`MIN_DAYS_FOR_PROJECTION = 3`，未達門檻時 `projectionReliable = false`，UI 顯示「—」。數字本身照算，只是不顯示。

**③ 收入不套用第一條規則。** 低估支出會高估安全感（危險），低估收入只會讓額度變小（安全）。而且收入通常是幾筆大額進帳，月份不完整也可能已經收完整。這個不對稱是刻意的。

### 緩衝資金的基準

有手動設定月預算時以它為基準，而不是推估收入——使用者已經明確表達「這個月我打算花多少」，那比系統的猜測更能代表意圖。

### 收入也有同樣的月初問題

預算是 `收入 × (1 − 目標儲蓄率)`。**月初薪水還沒入帳時，若直接用當月已記錄的收入，預算會被錨定在極低的數字上。**

實際踩到的案例（9/1）：只記了家人給的 20,000，薪水未入帳。預算變成 13,000，扣掉房租 7,000 與午餐 345、再扣尚未支付的固定支出 2,857，剩 2,798 除以 30 天 = **每天 93 元**。

原本的後備機制只在「當月收入完全是 0」時才啟動——**部分收入比完全沒有收入更糟**，因為後者至少會觸發後備。

修法跟固定支出對稱：`預期收入 = max(本月已記錄, 近三個月月均)`。同一個案例改用 35,186 推估後，每日額度變成 422 元。薪水入帳、實際超過歷史值之後就自動改用實際收入。UI 會標示「（依近期收入推估）」。

### 浮動的項目也要填

水電費每月不同，直覺上不該設成固定範本。但**金額會變不代表可以不付**——不填就等於每個月少扣那筆，額度持續高估。正確做法是填近期的概數，記帳當天的差額很小。**大致正確遠勝過完全不算。**

### 雙月繳費的項目（台電、部分網路費）

**填月均值**，例如雙月 2,400 就填 1,200。

- 非繳費月：系統預留 1,200，額度少約 40 元/天——那正好是在為下期帳單存錢
- 繳費月：記一筆 2,400，`max(0, 1200 − 2400)` 被 clamp 到 0 所以不會重複扣，但額度那個月會比較緊。**那個緊是誠實的**，現金確實那個月少了 2,400

**不要加「每 N 個月」的範本欄位。** 技術上可行，但體驗更差：非繳費月完全不預留，繳費月一次撞上整筆，比均攤更顛簸。

想完全平滑的話，繳費時把 2,400 拆成兩筆各 1,200 記在帳單涵蓋的兩個月——代價是要記到上個月的日期，過去月份的報表會被追溯修改。

### 尚未做

範本目前只用於計算，**不會自動產生交易**。SPEC 原本規劃的「每月一鍵產生草稿」還沒做，`RecurringRun` 表已經備好（`@@unique([templateId, yearMonth])` 保證冪等）。

## 8.14 上月結轉（previousMonthCarryover）

### 要解決的問題

「接下來每天可花」只算這個月：`(預算 − 已花消費) ÷ 剩餘天數`。但如果上個月沒花完、也沒投資出去，那筆錢仍然是現金——尤其**這個月收入斷檔**（剛換工作、還沒領到第一份薪水）時，系統應該知道上個月剩下的錢還能繼續用，而不是把每日額度算成近乎 0。

### 公式

```
上月結餘 = 上月收入 − 上月消費 − 上月儲蓄 − 上月投資
carryover = max(0, 上月結餘)
```

併入兩個既有數字：

```
每日可用額度：budgetRemaining = 預算 + carryover − 已花消費
緩衝／娛樂資金：buffer = 收入 + carryover − 固定支出 − 預估變動消費
```

### 兩個刻意的限制

**① 只看上一個月，不累加。** 長期累積的效果已經反映在「現金」與「緊急預備金」裡；`previousMonthCarryover` 是短期規劃用的數字，不是財富追蹤。如果疊代往前累加，「這個月能花多少」會跟實際現金餘額脫節，越滾越不透明，而且會跟緊急預備金的用途重疊。

**② 超支不會變成負的結轉。** 上個月刷爆（結餘為負）不該讓這個月的預算被懲罰性地扣掉——那個信號已經反映在現金餘額裡了，重複扣一次只會讓使用者看不懂數字怎麼來的。所以 clamp 在 0，沒有上月資料時回傳 `null`，呼叫端當成 0 處理。

### 緩衝資金卡的顯示條件也要跟著放寬

緩衝／娛樂資金卡原本只在 `totalIncome > 0` 時顯示。但**收入斷檔正是這個功能設計要處理的情境**——如果因為本月零收入就把卡片藏起來，使用者反而看不到「其實還有上月結餘可以用」這件事。顯示條件改成 `totalIncome > 0 || carryover > 0`。

### UI 標示來源

跟「（依近期收入推估）」一樣的原則：多出來的錢要讓使用者知道從哪來，不能讓數字突然變大卻不解釋。首頁預算說明與緩衝資金明細都會在 `carryover > 0` 時附加「+ 上月結餘 NT$X」。

### 同時保留「不含結轉」的純本月數值

只顯示併入結轉後的數字，會讓使用者看不出這個月本身的收支到底健不健康——結轉會把一個入不敷出的月份「墊」成看起來正常。所以 `monthPace` 與 `bufferFund` 的回傳值刻意保留兩套：

```
dailyAllowance / buffer               ← 併入結轉，「實際還能花多少」，當作主要數字
dailyAllowanceThisMonth / bufferThisMonth  ← 不含結轉，只看這個月自己的收支
```

`carryover > 0` 時才在 UI 額外顯示「不含上月結餘，這個月本身：X」；沒有結轉時兩者相等，顯示會是純噪音所以不顯示。這個純本月數值刻意**不 clamp 成 0**——可以是負的，而且負值本身就是有意義的資訊（「這個月本身入不敷出，是靠上月結轉撐過去的」），跟 `dailyAllowance`／`buffer` 遇到超支會 clamp／標紅是不同的用途：後者要告訴使用者「現在還能不能花」，前者要告訴使用者「這個月本身健不健康」。
...
| — | 緩衝／娛樂資金 | ✅ |
| — | 固定支出範本，修正每日可用額度在月初的高估 | ✅ |
| — | 金額遮罩（眼睛按鈕），狀態存 cookie 讓伺服器端就渲染成遮罩 | ✅ |
| — | PWA：`app/manifest.ts` + 圖示，可安裝成 Android WebAPK | ✅ 待部署後才能安裝 |
| — | 深色模式（cookie 存主題，`@custom-variant dark` 手動宣告） | ✅ |
| — | 設定頁排序：常用連結移到最上面，開始記帳前的資產收合到最下面 | ✅ |
| — | 上月結轉：收入斷檔的月份仍可動用上個月沒花完的錢 | ✅ |
