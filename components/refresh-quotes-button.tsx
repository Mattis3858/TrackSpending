  ...
  return (
    <div className="text-right">
      <button
        ...
        onClick={refresh}
        disabled={pending}
        className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 py-1.5 text-sm text-slate-600 dark:text-slate-300 transition-colors hover:bg-slate-50 disabled:opacity-50"
      >
        <svg
          viewBox="0 0 24 24"
          fill="none"
      ...
      {done && (
        <p className="mt-1.5 text-xs text-emerald-600 dark:text-emerald-400">已更新</p>
      )}
      {error && (
